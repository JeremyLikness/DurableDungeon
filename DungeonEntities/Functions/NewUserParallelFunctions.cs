...
namespace DungeonEntities.Functions
{
    public static class NewUserParallelFunctions
    {
            ...
            ILogger logger)
        {
            ...
            await client.SignalEntityAsync(id, nameof(Monster.New), monster.Name);
            ...
            ILogger logger)
        {
            ...
            await client.SignalEntityAsync(id, nameof(Room.New), room.Name);
            await client.SignalEntityAsync(id, nameof(Room.SetDescription), room.Description);
            ...
            ILogger logger)
        {
            ...
            await client.SignalEntityAsync(id, nameof(InventoryList.New), inventoryList);
            foreach(var item in inventoryList)
            {
                id = username.AsEntityIdFor<Inventory>(item.Name);
                await client.SignalEntityAsync(id, nameof(Inventory.New), item.Name);
                if (item.IsTreasure)
                {
                    await client.SignalEntityAsync(id, nameof(Inventory.SetTreasure));
                }
            }
